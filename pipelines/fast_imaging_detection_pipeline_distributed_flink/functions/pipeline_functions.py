...
def serialize_object(object_in):
    pickled_object = pickle.dumps(object_in, protocol=5)
    serialized_object = pybase64.b64encode(pickled_object).decode('utf-8')
    return serialized_object

def hadoop_index_location(hdfs_folder, index):
    """ Function mapping indices to files in the hadoop distributed file system"""
    return str("webhdfs://namenode:9870") + str("/fast_imaging/") + str(hdfs_folder)+ str("/time_slice_") +str(index)

def write_hadoop(object_in, hdfs_folder, index):
    """ Pickle and object and save in the hadoop distributed file system, using webHDFS"""
    # hdfs_folder is the folder on hadoop where the slices are put
    pickled_object = pickle.dumps(object_in, protocol=5)
    with open(hadoop_index_location(hdfs_folder, index), 'wb') as fout:
        fout.write(pickled_object)

def read_hadoop(hdfs_folder, index):
    """ Load object from the hadoop distributed file system, using webHDFS and unpickle it"""
    with open(hadoop_index_location(hdfs_folder, index), 'rb') as fin:
        data_out = fin.read()
    object_out = pickle.loads(data_out)
    return object_out
