...
def run_ms2dirty(do_single, do_w_stacking, vis, model, freqs, uvw, weight, flags, im_size, pixel_size_arcsec, epsilon=1e-12):
...
def main():
    ...
    parser.add_option('--pixel_size_arcsec', dest = 'pixel_size_arcsec', default = 1.0, help = 'Pixel size (default = 1.0 arcsec)')
    parser.add_option('--do_single', dest = 'do_single', default = False, help = 'Make inverts in a single precision (default = False)')
    parser.add_option('--epsilon', dest = 'epsilon', default = 1e-12, help = 'Accuracy parameter (default = 1e-12 for FP64, should be 1e-5 for FP32)')
    parser.add_option('--do_w_stacking', dest = 'do_w_stacking', default = True, help = 'To perform w-stacking gridding (default = True)')
    ...
    for i in range(len(mstime_unique)):
        ...
        print("\n", i, mstime_unique[i], istart, iend, number_of_rows_to_read, vis_snap.shape, freqs.shape, uvw_snap.shape, "\n")
        dirty_image = run_ms2dirty(do_single, do_w_stacking, vis_snap, model_snap, freqs, uvw_snap, weight_snap, flags_snap, im_size, pixel_size_arcsec, epsilon=epsilon)
