...
from pipeline_functions import serialize_object, producer_open_ms, producer_send_slice
logging.basicConfig(
...
def write_data(producer, args, do_single, epsilon, do_w_stacking, im_size, pixel_size_arcsec):
    ...
    for i in range(len(mstime_unique)):
        ...
        data_b64 = serialize_object(slice_data)

        # Add timestamp and send data
        ts = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        cur_data = {"createTime": ts, "serialData": data_b64}
